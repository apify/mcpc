...
start_test_server() {
  ...
  # Start server
  cd "$PROJECT_ROOT"
  env $env_str $TSX test/e2e/server/index.ts >"$_TEST_RUN_DIR/server.log" 2>&1 &
  _TEST_SERVER_PID=$!
...
#
# Runs the locally installed @modelcontextprotocol/server-filesystem via node
# rather than `npx -y`, which would resolve (and on a cold cache download) the
# package on every stdio server spawn. Invoking the entrypoint through node also
# keeps this portable: the .bin wrapper is a shell script that Windows cannot
# execute directly.
create_fs_config() {
  local path="${1:-$TEST_TMP}"
  local entry="$PROJECT_ROOT/node_modules/@modelcontextprotocol/server-filesystem/dist/index.js"

  if [[ -f "$entry" ]]; then
    # create_stdio_config converts absolute paths to native form for us
    create_stdio_config "fs" "node" "$entry" "$path"
  else
    # Fall back to npx when the package is not installed locally
