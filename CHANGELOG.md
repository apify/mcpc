...
- Windows logins no longer fail with `Attribute 'password encoded as UTF-16' is longer than platform limit of 2560 chars`. Credentials that exceed what Windows Credential Manager accepts in one entry — most OAuth token blobs — are now split across several entries, so `login` and background token refresh work with any token size. (#409)
